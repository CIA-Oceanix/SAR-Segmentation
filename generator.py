    ...
    while True:
        ...
        for i, (input_, output, angle, zoom, h_flip) in \
            ...
            if batch_image_input.shape[-1] != 1:
                batch_image_input[i] = cv2.warpAffine(input_, input_rotation_matrix, input_shape[:2][::-1])
            else:
                batch_image_input[i, :, :, 0] = cv2.warpAffine(input_, input_rotation_matrix, input_shape[:2][::-1])
...
def get_add_additional_inputs(root, attributes):
    ...
    with open(os.path.join(root, 'output.json'), 'r') as json_file:
        additional_inputs = json.load(json_file)
    with open(os.path.join(root, 'normalization.json'), 'r') as json_file:
        normalization_dict = json.load(json_file)

    additional_input_number = len(attributes)
    normalized_inputs = {}
    for attribute_name, normalization in normalization_dict.items():
        ...
        for filename, entry in additional_inputs.items():
            if filename not in normalized_inputs:
                normalized_inputs[filename] = {}
            value = entry[attribute_name]
            if np.isscalar(value) and -10 ** 10 < value < 10 ** 10:
                normalized_inputs[filename][attribute_name] = (value - mean) / std
            else:
                normalized_inputs[filename][attribute_name] = 0

    def add_additional_inputs(batch_input, batch_input_path):
        additional_inputs_array = np.zeros((batch_input.shape[0], additional_input_number))
