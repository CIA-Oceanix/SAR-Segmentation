...
def training_schedule(
    ...
    lod_training_kimg       = lod_training_kimg,   # Thousands of real images to show before doubling the resolution.
    ...
    tick_kimg_base          = tick_kimg_base,      # Default interval of progress snapshots.
    tick_kimg_dict          = tick_kimg_dict):     # Resolution-specific overrides.
...
def training_loop(
    ...
    image_snapshot_ticks    = image_snapshot_ticks,                   # How often to export image snapshots?
    network_snapshot_ticks  = network_snapshot_ticks,                 # How often to export network snapshots?
    save_tf_graph           = False,                                  # Include full TensorFlow computation graph in the tfevents file?
    save_weight_histograms  = False,                                  # Include weight histograms in the tfevents file?
    resume_run_id           = resume_run_id,                          # Run ID or network pkl to resume training from, None = start from scratch.
    ...
    with tf.device('/gpu:0'):
        try:
            peak_gpu_mem_op = tf.constant(4*(2**30))#tf.contrib.memory_stats.MaxBytesInUse()
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            # Report progress.
            print('%s - tick %-5d kimg %-8.1f lod %-5.2f minibatch %-4d time %-12s sec/tick %-7.1f sec/kimg %-7.2f maintenance %-6.1f gpumem %-4.1f' % (
