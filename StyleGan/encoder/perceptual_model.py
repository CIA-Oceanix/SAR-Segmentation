...
class PerceptualModel:
    ...
    def get_features_from_latent(self, latent_vector, shape, data_type):
        latent_vector = np.frombuffer(latent_vector, dtype=data_type).reshape(shape)
        generator = self.generator
        im = generator.generate_image(latent_vector)
        im = np.expand_dims(im, 0)
        im_features = self.perceptual_model.predict(im, steps=1)[0]
        return im_features

    def diff_images(self, latent1, latent2):
