...
def plot_less_than_three_canals(input_, prediction, groundtruth, labels):
    ...
    for i, (input_thumb, pred_thumb, truth_thumb) in enumerate(zip(input_, prediction, groundtruth)):
        ...
        imshow(pred_thumb, vmin=0, vmax=groundtruth.max(), cmap='hot', threshold = 'auto')
        ...
        if line_number == 3:
            ...
            imshow(truth_thumb[:, :, :min(truth_thumb.shape[-1], 3)], vmin=0, vmax=groundtruth.max(), cmap='hot')
